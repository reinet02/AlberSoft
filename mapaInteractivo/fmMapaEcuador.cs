    ...
    public partial class fmMapaEcuador : Form
    {
        ...
        private async Task InicializarYMostrarMapaAsync()
        {
            try
            {
                ...
                // Registrar un handler para cuando el DOM esté cargado; así nos aseguramos
                // de que las funciones JS ya estén definidas antes de intentar inyectar el SVG.
                vistaWeb.CoreWebView2.DOMContentLoaded += ManejarDOMContentCargadoAsync;
            }
            catch (Exception ex)
            {
        ...
        // Handler para DOMContentLoaded. Es async porque ejecuta un script en la página.
        private async void ManejarDOMContentCargadoAsync(object? sender, Microsoft.Web.WebView2.Core.CoreWebView2DOMContentLoadedEventArgs e)
        {
            try
            {
                ...
                {
                    // Leer el SVG como texto y pasarlo al HTML usando una función JS llamada recibirSvg
                    var svgText = File.ReadAllText(svgPath);
                    var json = JsonSerializer.Serialize(svgText);

                    // A veces NavigationCompleted ocurre antes de que el script defina window.recibirSvg.
                    // Intentamos detectar la función y reintentar unos cuantos ciclos antes de aplicar
                    // un fallback que deje el SVG en una variable global y llame a recibirSvg si existe.
                    bool inyectado = false;
                    try
                    {
                        for (int intento = 0; intento < 20; intento++)
                        {
                            try
                            {
                                var check = await vistaWeb.CoreWebView2.ExecuteScriptAsync("typeof window.recibirSvg === 'function'");
                                if (!string.IsNullOrEmpty(check) && check.IndexOf("true", StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                    ...
                    if (!inyectado)
                    {
                        // Fallback: guardar contenido en variable global y llamar a recibirSvg si ya existe
                        var fallbackScript = $@"(function(){{
                            try {{ window._svg_inyectado_por_host = {json}; if (typeof window.recibirSvg === 'function') window.recibirSvg(window._svg_inyectado_por_host); }} catch(e) {{ /* ignore */ }}
                        }})();";
                        try { await vistaWeb.CoreWebView2.ExecuteScriptAsync(fallbackScript); } catch { }
                    }
                }
                else
                {
