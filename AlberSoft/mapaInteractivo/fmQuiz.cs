namespace AlberSoft.mapaInteractivo
{
    ...
    public partial class fmQuiz : Form
    {
        ...
        public fmQuiz(int nivel)
        {
            ...
            // Mostrar el nivel seleccionado al usuario (ej. "Nivel: Difícil")
            string textoNivel = nivel == 1 ? "Fácil" : nivel == 2 ? "Intermedio" : "Difícil";
            if (lbNivel != null)
            {
                lbNivel.Content = $"Nivel: {textoNivel}";
            }


            // Inicializar tiempo total según el nivel (no arrancar el temporizador aún)
            tiempoRestante = nivelSeleccionado == 1 ? 300 : nivelSeleccionado == 2 ? 180 : 120; // segundos
            if (labelTiempo != null)
            {
        ...
        // Inicia el temporizador del quiz (llamar cuando el usuario presiona "Jugar")
        ...
        // Aplica una penalización en segundos al tiempo restante y muestra la penalización en pantalla
        private void AplicarPenalizacion(int segundos)
        {
            ...
            // Mostrar penalización al usuario
            if (lbPenalizacion != null)
            {
                lbPenalizacion.Content = $"- {segundos} segundos";
                // Iniciar temporizador que ocultará la penalización después de 5s
                if (temporizadorPenalizacion != null)
                {
                    temporizadorPenalizacion.Stop();
                    temporizadorPenalizacion.Start();
                }
            }

            // Si el tiempo llegó a 0, finalizar/avanzar como tiempo agotado
            if (tiempoRestante == 0)
            {
        ...
        // Timer que oculta la etiqueta de penalización
        private void ManejarTickPenalizacion(object? sender, EventArgs e)
        {
            if (temporizadorPenalizacion != null)
            {
                temporizadorPenalizacion.Stop();
        ...
        private void MostrarPreguntaActual()
        {
            ...
            // Mostrar el tiempo restante actual (no reiniciamos el temporizador al cambiar de pregunta)
            if (labelTiempo != null)
            {
                labelTiempo.Content = FormatearTiempo(tiempoRestante);
            }

            // Mostrar índice de pregunta al usuario (ej. "Pregunta 1/10")
            int total = preguntas.Count;
            int actualMostrar = total > 0 ? Math.Min(indiceActual + 1, total) : 0;
            if (lbIndicePregunta != null)
            {
                lbIndicePregunta.Content = $"Pregunta {actualMostrar}/{total}";
            }
        }
        #endregion
        ...
        private void Opcion_Click(object? sender, EventArgs e)
        {
            ...
            // No detener el temporizador al responder; debe continuar entre preguntas
            ...
            else
            {
                ...
                // Aplicar penalización según el nivel
                int penalizacionSegundos = nivelSeleccionado == 1 ? 5 : nivelSeleccionado == 2 ? 10 : 15;
                AplicarPenalizacion(penalizacionSegundos);
