...
namespace AlberSoft
{
    public partial class fmMenu : Form
    {
        ...
        // Indica que el usuario ya confirmó la salida para evitar mostrar el MessageBox varias veces
        ...
        public fmMenu()
        {
            ...
            // Cuando el formulario se cierre, preguntar al usuario si realmente desea salir
            // Cancelar el cierre si el usuario responde que no
            FormClosing += fmMenu_FormClosing;
        }


        // No es necesario poner esto en los demás formularios porque en este formulario
        // se llaman a los demas (embebido)
        private void fmMenu_FormClosing(object? sender, FormClosingEventArgs e)
        {
            // Si ya se confirmó la salida, no volver a preguntar
            // fue necesari
            if (salirConfirmado) return;

            var result = MessageBox.Show("¿Está seguro de que desea salir del programa?", "Confirmar salida", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            // Usuario confirmó la salida: marcar para no volver a preguntar y permitir el cierre normal
            salirConfirmado = true;

            // Cerrar la aplicación completamente (esto cerrará todos los formularios abiertos)
            Application.Exit();
        }
        ...
        private ContextMenuStrip CrearMenuFlotante2()
        {
            ...
            for (int i = 0; i < labels2.Length; i++)
            {
                ...
                item.Click += (s, e) =>
                {
                    if (label == "Emergencias")
                    {
                        abrirFormularioHijo(new fmCasosEmergencia());
                    }
                    else if (label == "Juego formativo")
                    {
                        abrirFormularioHijo(new menuPrincipalJuegoCartas());
                    }
                    else
                    {
                        noAsignado_Click(s, e);
